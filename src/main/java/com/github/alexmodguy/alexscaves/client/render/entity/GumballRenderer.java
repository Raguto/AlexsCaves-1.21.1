...
public class GumballRenderer extends EntityRenderer<GumballEntity> {
    ...
    public void render(GumballEntity entity, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource multiBufferSource, int packedLight) {
        ...
        vertex(vertexconsumer, matrix4f, posestack$pose, packedLight, 0.0F, 1, 0, 0, 1F);
        if(entity.isExplosive()){
            ...
            vertex(vertexconsumer2, matrix4f, posestack$pose, packedLight, 0.0F, 0, 0, 1, explodeColorChange);
            vertex(vertexconsumer2, matrix4f, posestack$pose, packedLight, 1.0F, 0, 1, 1, explodeColorChange);
            vertex(vertexconsumer2, matrix4f, posestack$pose, packedLight, 1.0F, 1, 1, 0, explodeColorChange);
            vertex(vertexconsumer2, matrix4f, posestack$pose, packedLight, 0.0F, 1, 0, 0, explodeColorChange);
        }
        poseStack.popPose();
        super.render(entity, entityYaw, partialTicks, poseStack, multiBufferSource, packedLight);
    }

    private static void vertex(VertexConsumer vertexConsumer, Matrix4f matrix4f, PoseStack.Pose pose, int p_253829_, float x, int y, int u, int v, float alpha) {
        vertexConsumer.addVertex(matrix4f, x - 0.5F, (float)y - 0.25F, 0.0F).setColor(1F, 1F, 1F, alpha).setUv((float)u, (float)v).setOverlay(OverlayTexture.NO_OVERLAY).setLight(p_253829_).setNormal(pose, 0.0F, 1.0F, 0.0F);
    }
