...
public class VoidBeingTendrilParticle extends Particle {
    ...
    public void render(VertexConsumer vertexConsumer, Camera camera, float partialTick) {
        ...
        while (samples <= sampleCount) {
            ...
            vertexconsumer.addVertex(matrix4f, (float) drawFrom.x + (float) bottomAngleVec.x, (float) drawFrom.y + (float) bottomAngleVec.y, (float) drawFrom.z + (float) bottomAngleVec.z).setColor(255, 255, 255, (int)(startA * 255)).setUv(u1, 1F).setOverlay(NO_OVERLAY).setUv2(j & 0xFFFF, j >> 16).setNormal(posestack$pose, 0.0F, 1.0F, 0.0F);
            vertexconsumer.addVertex(matrix4f, (float) drawTo.x + (float) bottomAngleVec.x, (float) drawTo.y + (float) bottomAngleVec.y, (float) drawTo.z + (float) bottomAngleVec.z).setColor(255, 255, 255, (int)(endA * 255)).setUv(u2, 1F).setOverlay(NO_OVERLAY).setUv2(j & 0xFFFF, j >> 16).setNormal(posestack$pose, 0.0F, 1.0F, 0.0F);
            vertexconsumer.addVertex(matrix4f, (float) drawTo.x + (float) topAngleVec.x, (float) drawTo.y + (float) topAngleVec.y, (float) drawTo.z + (float) topAngleVec.z).setColor(255, 255, 255, (int)(endA * 255)).setUv(u2, 0).setOverlay(NO_OVERLAY).setUv2(j & 0xFFFF, j >> 16).setNormal(posestack$pose, 0.0F, 1.0F, 0.0F);
            vertexconsumer.addVertex(matrix4f, (float) drawFrom.x + (float) topAngleVec.x, (float) drawFrom.y + (float) topAngleVec.y, (float) drawFrom.z + (float) topAngleVec.z).setColor(255, 255, 255, (int)(startA * 255)).setUv(u1, 0).setOverlay(NO_OVERLAY).setUv2(j & 0xFFFF, j >> 16).setNormal(posestack$pose, 0.0F, 1.0F, 0.0F);
            samples++;
            drawFrom = drawTo;
        }
