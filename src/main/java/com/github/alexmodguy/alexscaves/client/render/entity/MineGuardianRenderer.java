...
public class MineGuardianRenderer extends MobRenderer<MineGuardianEntity, MineGuardianModel> {
    ...
    private static void shineOriginVertex(VertexConsumer p_114220_, Matrix4f p_114221_, PoseStack.Pose pose, float xOffset, float yOffset) {
        p_114220_.addVertex(p_114221_, 0.0F, 0.0F, 0.0F).setColor(230, 0, 0, 230).setUv(xOffset + 0.5F, yOffset).setOverlay(NO_OVERLAY).setLight(240).setNormal(pose, 0.0F, 1.0F, 0.0F);
    }
    ...
    private static void shineLeftCornerVertex(VertexConsumer p_114215_, Matrix4f p_114216_, PoseStack.Pose pose, float p_114217_, float p_114218_, float xOffset, float yOffset) {
        p_114215_.addVertex(p_114216_, -ACMath.HALF_SQRT_3 * p_114218_, p_114217_, 0).setColor(255, 0, 0, 0).setUv(xOffset, yOffset + 1).setOverlay(NO_OVERLAY).setLight(240).setNormal(pose, 0.0F, -1.0F, 0.0F);
    }
    ...
    private static void shineRightCornerVertex(VertexConsumer p_114224_, Matrix4f p_114225_, PoseStack.Pose pose, float p_114226_, float p_114227_, float xOffset, float yOffset) {
        p_114224_.addVertex(p_114225_, ACMath.HALF_SQRT_3 * p_114227_, p_114226_, 0).setColor(255, 0, 0, 0).setUv(xOffset + 1, yOffset + 1).setOverlay(NO_OVERLAY).setLight(240).setNormal(pose, 0.0F, -1.0F, 0.0F);
    }
    ...
    class LayerGlow extends RenderLayer<MineGuardianEntity, MineGuardianModel> {
        ...
        public void render(PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn, MineGuardianEntity entitylivingbaseIn, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
            ...
            if (!entitylivingbaseIn.isEyeClosed()) {
                ...
                this.getParentModel().renderToBuffer(poseStack, ivertexbuilder1, packedLightIn, LivingEntityRenderer.getOverlayCoords(entitylivingbaseIn, 0.0F), 0xFFFFFFFF);
            }
            VertexConsumer ivertexbuilder4 = bufferIn.getBuffer(ACRenderTypes.getEyesAlphaEnabled(TEXTURE_EXPLODE));
            int argb = ((int)(explodeProgress * 255) << 24) | 0xFFFFFF;
            this.getParentModel().renderToBuffer(poseStack, ivertexbuilder4, packedLightIn, LivingEntityRenderer.getOverlayCoords(entitylivingbaseIn, 0.0F), argb);
