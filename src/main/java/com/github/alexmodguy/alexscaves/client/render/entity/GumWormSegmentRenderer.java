...
public class GumWormSegmentRenderer extends EntityRenderer<GumWormSegmentEntity> {
    ...
    public void render(GumWormSegmentEntity entity, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn) {
        ...
        MODEL.renderToBuffer(poseStack, ivertexbuilder, packedLightIn, getOverlayCoords(entity, 0.0F), 0xFFFFFFFF);
    ...
    public static void renderGum(Vec3 to, PoseStack poseStack, VertexConsumer gumConsumer, int packedLightIn, int overlayCoords) {
        ...
        gumConsumer.addVertex(matrix4f, 0, 0, -0.5F).setColor(255, 255, 255, 255).setUv((float) 0, (float) 0).setOverlay(overlayCoords).setLight(packedLightIn).setNormal(posestack$pose, 0.0F, -1.0F, 0.0F);
        gumConsumer.addVertex(matrix4f, 0, 0, 0.5F).setColor(255, 255, 255, 255).setUv((float) 1, (float) 0).setOverlay(overlayCoords).setLight(packedLightIn).setNormal(posestack$pose, 0.0F, -1.0F, 0.0F);
        gumConsumer.addVertex(matrix4f, 0, length, 0.5F).setColor(255, 255, 255, 255).setUv((float) 1, (float)1).setOverlay(overlayCoords).setLight(packedLightIn).setNormal(posestack$pose, 0.0F, -1.0F, 0.0F);
        gumConsumer.addVertex(matrix4f, 0, length, -0.5F).setColor(255, 255, 255, 255).setUv((float) 0, (float)1).setOverlay(overlayCoords).setLight(packedLightIn).setNormal(posestack$pose, 0.0F, -1.0F, 0.0F);
        poseStack.popPose();
    }
