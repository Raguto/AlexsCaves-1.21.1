...
public class QuarrySmasherRenderer extends EntityRenderer<QuarrySmasherEntity> {
    ...
    public void render(QuarrySmasherEntity entity, float entityYaw, float partialTicks, PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn) {
        ...
        QUARRY_SMASHER_MODEL.renderToBuffer(poseStack, textureVertexConsumer, packedLightIn, OverlayTexture.NO_OVERLAY, 0xFFFFFFFF);
        VertexConsumer activeVertexConsumer = bufferIn.getBuffer(ACRenderTypes.getEyesAlphaEnabled(TEXTURE_GLOW));
        int argb = ((int)((1F - inactive) * 255) << 24) | 0xFFFFFF;
        QUARRY_SMASHER_MODEL.renderToBuffer(poseStack, activeVertexConsumer, 240, OverlayTexture.NO_OVERLAY, argb);
        poseStack.popPose();
        ...
        if (entity.headPart != null && entity.tickCount > 0) {
            ...
            BOUNDROID_MODEL.renderToBuffer(poseStack, boundroidVertexConsumer, packedLightIn, OverlayTexture.NO_OVERLAY, 0xFFFFFFFF);
            poseStack.popPose();
