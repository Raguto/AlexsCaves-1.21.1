...
public class VallumraptorModel extends AdvancedEntityModel<VallumraptorEntity> {
    ...
    public void renderToBuffer(PoseStack matrixStackIn, VertexConsumer bufferIn, int packedLightIn, int packedOverlayIn, int packedColor) {
        // Extract alpha from packed color and multiply with model alpha
        float alphaIn = FastColor.ARGB32.alpha(packedColor) / 255.0F;
        int adjustedColor = FastColor.ARGB32.color((int)(alphaIn * this.alpha * 255), FastColor.ARGB32.red(packedColor), FastColor.ARGB32.green(packedColor), FastColor.ARGB32.blue(packedColor));
        if (this.young) {
            float f = 1.5F;
            head.setScale(f, f, f);
            ...
            parts().forEach((p_228292_8_) -> {
                p_228292_8_.render(matrixStackIn, bufferIn, packedLightIn, packedOverlayIn, adjustedColor);
            });
            ...
            parts().forEach((p_228290_8_) -> {
                p_228290_8_.render(matrixStackIn, bufferIn, packedLightIn, packedOverlayIn, adjustedColor);
            });
            matrixStackIn.popPose();
        }
    }
