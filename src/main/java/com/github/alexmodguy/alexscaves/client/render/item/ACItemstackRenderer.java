...
public class ACItemstackRenderer extends BlockEntityWithoutLevelRenderer {
    ...
    // Helper method to create packed ARGB color with variable alpha
    private static int colorWithAlpha(float alpha) {
        int a = (int)(alpha * 255.0F) & 0xFF;
        return (a << 24) | 0x00FFFFFF; // White with variable alpha
    }
    ...
    public void renderByItem(ItemStack itemStackIn, ItemDisplayContext transformType, PoseStack poseStack, MultiBufferSource bufferIn, int combinedLightIn, int combinedOverlayIn) {
        ...
        if (itemStackIn.is(ACItemRegistry.GALENA_GAUNTLET.get())) {
            ...
            if (left || transformType == ItemDisplayContext.GUI) {
                ...
                GALENA_GAUNTLET_LEFT_MODEL.renderToBuffer(poseStack, getVertexConsumer(bufferIn, ACRenderTypes.getEyesAlphaEnabled(GALENA_GAUNTLET_BLUE_TEXTURE), GALENA_GAUNTLET_BLUE_TEXTURE), combinedLightIn, combinedOverlayIn, colorWithAlpha(openAmount));
                GALENA_GAUNTLET_LEFT_MODEL.renderToBuffer(poseStack, getVertexConsumer(bufferIn, ACRenderTypes.getEyesAlphaEnabled(GALENA_GAUNTLET_RED_TEXTURE), GALENA_GAUNTLET_RED_TEXTURE), combinedLightIn, combinedOverlayIn, colorWithAlpha(closeAmount));
            } else {
                ...
                GALENA_GAUNTLET_RIGHT_MODEL.renderToBuffer(poseStack, getVertexConsumer(bufferIn, ACRenderTypes.getEyesAlphaEnabled(GALENA_GAUNTLET_BLUE_TEXTURE), GALENA_GAUNTLET_BLUE_TEXTURE), combinedLightIn, combinedOverlayIn, colorWithAlpha(openAmount));
                GALENA_GAUNTLET_RIGHT_MODEL.renderToBuffer(poseStack, getVertexConsumer(bufferIn, ACRenderTypes.getEyesAlphaEnabled(GALENA_GAUNTLET_RED_TEXTURE), GALENA_GAUNTLET_RED_TEXTURE), combinedLightIn, combinedOverlayIn, colorWithAlpha(closeAmount));
            }
            poseStack.popPose();
        }
        ...
        if (itemStackIn.is(ACItemRegistry.RESISTOR_SHIELD.get())) {
            ...
            RESISTOR_SHIELD_MODEL.renderToBuffer(poseStack, getVertexConsumer(bufferIn, ACRenderTypes.getEyesAlphaEnabled(RESISTOR_SHIELD_RED_TEXTURE), RESISTOR_SHIELD_RED_TEXTURE), combinedLightIn, combinedOverlayIn, colorWithAlpha(switchProgress));
            RESISTOR_SHIELD_MODEL.renderToBuffer(poseStack, getVertexConsumer(bufferIn, ACRenderTypes.getEyesAlphaEnabled(RESISTOR_SHIELD_BLUE_TEXTURE), RESISTOR_SHIELD_BLUE_TEXTURE), combinedLightIn, combinedOverlayIn, colorWithAlpha(1.0F - switchProgress));
            poseStack.popPose();
        }
        ...
        if (itemStackIn.is(ACItemRegistry.EXTINCTION_SPEAR.get())) {
            ...
            if (heldIn3d) {
                ...
                VertexConsumer vertexconsumer2 = ItemRenderer.getArmorFoilBuffer(bufferIn, NeoForgeRenderTypes.getUnlitTranslucent(EXTINCTION_SPEAR_TEXTURE), itemStackIn.hasFoil());
        ...
        if (itemStackIn.is(ACBlockRegistry.SIREN_LIGHT.get().asItem())) {
            ...
            SIREN_LIGHT_MODEL.renderToBuffer(poseStack, getVertexConsumer(bufferIn, RenderType.entityTranslucent(SIREN_LIGHT_COLOR_TEXTURE), SIREN_LIGHT_COLOR_TEXTURE), combinedLightIn, combinedOverlayIn, 0xFF00FF00);
            poseStack.popPose();
        }
        ...
        if (itemStackIn.is(ACItemRegistry.RAYGUN.get())) {
            ...
            boolean gamma = level != null && net.minecraft.world.item.enchantment.EnchantmentHelper.getItemEnchantmentLevel(level.holderOrThrow(ACEnchantmentRegistry.GAMMA_RAY), itemStackIn) > 0;
            ...
            RAYGUN_MODEL.renderToBuffer(poseStack, getVertexConsumer(bufferIn, ACRenderTypes.getEyesAlphaEnabled(textureActive), textureActive), combinedLightIn, combinedOverlayIn, colorWithAlpha(pulseAlpha));
            poseStack.popPose();
            poseStack.popPose();
        }
        ...
        if (itemStackIn.is(ACItemRegistry.DREADBOW.get())) {
            float ageInTicks = Minecraft.getInstance().player == null ? 0F : Minecraft.getInstance().player.tickCount + partialTick;
            float pullAmount = DreadbowItem.getPullingAmount(itemStackIn, partialTick, Minecraft.getInstance().player);
            poseStack.translate(0.5F, 0.5F, 0.5F);
