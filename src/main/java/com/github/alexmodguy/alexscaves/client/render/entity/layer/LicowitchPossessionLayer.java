    ...
    public void render(PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn, T entity, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        if (entity.getPossessedByLicowitchId() != -1) {
            ...
            if (licowitch != null) {
                ...
                int argb1 = ((int)(possessAlpha * 255) << 24) | ((int)(1.0F * 255) << 16) | ((int)(0.3F * 255) << 8) | (int)(1.0F * 255);
                model.renderToBuffer(poseStack, bufferIn.getBuffer(RenderType.entityTranslucent(texture)), packedLightIn, LivingEntityRenderer.getOverlayCoords(entity, 0.0F), argb1);
                model.renderToBuffer(poseStack, bufferIn.getBuffer(ACRenderTypes.getPurpleWitch(texture)), packedLightIn, LivingEntityRenderer.getOverlayCoords(entity, 0), 0xFFFFFFFF);
            }
        }
    }
}
