...
public class FerrouslimeRenderer extends EntityRenderer<FerrouslimeEntity> implements CustomBookEntityRenderer {
    ...
    private void renderHead(FerrouslimeEntity entity, float partialTicks, PoseStack poseStack, MultiBufferSource source, int packedLight) {
        ...
        FERROUSLIME_MODEL.renderToBuffer(poseStack, textureConsumer, packedLight, LivingEntityRenderer.getOverlayCoords(entity, 0.0F), 0xFFFFFFFF);
        ...
        FERROUSLIME_MODEL.renderToBuffer(poseStack, eyesConsumer, packedLight, LivingEntityRenderer.getOverlayCoords(entity, 0.0F), 0xFFFFFFFF);
        poseStack.popPose();
    }
    ...
    private void renderGelSpikes(FerrouslimeEntity entity, float partialTicks, PoseStack poseStack, VertexConsumer consumer, float size, int packedLight) {
        ...
        if (length > 0) {
            for (int i = 0; i < 8; i++) {
                ...
                this.spikeCubeFace(entity, cubeAt, pose, consumer, size * 0.25F, length, size * 0.25F, packedLight);
                poseStack.popPose();
            }
        }
    }

    private void spikeCubeFace(FerrouslimeEntity entity, Matrix4f matrix4f, PoseStack.Pose pose, VertexConsumer vertexConsumer, float offset, float length, float width, int packedLightIn) {
        ...
        vertexConsumer.addVertex(matrix4f, 0, 0, offset + length).setColor(255, hurtColor, hurtColor, 255).setUv((float) 0, (float) 0).setOverlay(overlayCoords).setLight(packedLightIn).setNormal(pose, 0.0F, -1.0F, 0.0F);
        vertexConsumer.addVertex(matrix4f, 0, width, offset).setColor(255, hurtColor, hurtColor, 255).setUv((float) width, (float) length).setOverlay(overlayCoords).setLight(packedLightIn).setNormal(pose, 0.0F, -1.0F, 0.0F);
        vertexConsumer.addVertex(matrix4f, 0, -width, offset).setColor(255, hurtColor, hurtColor, 255).setUv((float) 0, (float) length).setOverlay(overlayCoords).setLight(packedLightIn).setNormal(pose, 0.0F, -1.0F, 0.0F);
    ...
    private void renderGel(FerrouslimeEntity entity, float partialTicks, PoseStack poseStack, VertexConsumer consumer, float size, int packedLight) {
        ...
        float spike = 1;
        this.renderCubeFace(entity, cubeAt, pose, consumer, packedLight, cubeStart, cubeEnd, cubeStart, cubeEnd, cubeEnd, cubeEnd, cubeEnd, cubeEnd, textureScale);
        this.renderCubeFace(entity, cubeAt, pose, consumer, packedLight, cubeStart, cubeEnd, cubeEnd, cubeStart, cubeStart, cubeStart, cubeStart, cubeStart, textureScale);
        this.renderCubeFace(entity, cubeAt, pose, consumer, packedLight, cubeEnd, cubeEnd, cubeEnd, cubeStart, cubeStart, cubeEnd, cubeEnd, cubeStart, textureScale);
        this.renderCubeFace(entity, cubeAt, pose, consumer, packedLight, cubeStart, cubeStart, cubeStart, cubeEnd, cubeStart, cubeEnd, cubeEnd, cubeStart, textureScale);
        this.renderCubeFace(entity, cubeAt, pose, consumer, packedLight, cubeStart, cubeEnd, cubeStart, cubeStart, cubeStart, cubeStart, cubeEnd, cubeEnd, textureScale);
        this.renderCubeFace(entity, cubeAt, pose, consumer, packedLight, cubeStart, cubeEnd, cubeEnd, cubeEnd, cubeEnd, cubeEnd, cubeStart, cubeStart, textureScale);
    }

    private void renderCubeFace(FerrouslimeEntity entity, Matrix4f matrix4f, PoseStack.Pose pose, VertexConsumer vertexConsumer, int packedLightIn, float f1, float f2, float f3, float f4, float f5, float f6, float f7, float f8, float textureScale) {
        int overlayCoords = LivingEntityRenderer.getOverlayCoords(entity, 0.0F);
        int hurtColor = entity.hurtTime > 0 || entity.deathTime > 0 ? 10 : 255;
        vertexConsumer.addVertex(matrix4f, f1, f3, f5).setColor(255, hurtColor, hurtColor, 255).setUv((float) 0, (float) textureScale).setOverlay(overlayCoords).setLight(packedLightIn).setNormal(pose, 0.0F, -1.0F, 0.0F);
        vertexConsumer.addVertex(matrix4f, f2, f3, f6).setColor(255, hurtColor, hurtColor, 255).setUv((float) textureScale, (float) textureScale).setOverlay(overlayCoords).setLight(packedLightIn).setNormal(pose, 0.0F, -1.0F, 0.0F);
        vertexConsumer.addVertex(matrix4f, f2, f4, f7).setColor(255, hurtColor, hurtColor, 255).setUv((float) textureScale, (float) 0).setOverlay(overlayCoords).setLight(packedLightIn).setNormal(pose, 0.0F, -1.0F, 0.0F);
        vertexConsumer.addVertex(matrix4f, f1, f4, f8).setColor(255, hurtColor, hurtColor, 255).setUv((float) 0, (float) 0).setOverlay(overlayCoords).setLight(packedLightIn).setNormal(pose, 0.0F, -1.0F, 0.0F);
    }
