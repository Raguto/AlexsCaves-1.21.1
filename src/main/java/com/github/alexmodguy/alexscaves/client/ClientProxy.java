...
public class ClientProxy extends CommonProxy {
    ...
    private void registerMenuScreens(net.neoforged.neoforge.client.event.RegisterMenuScreensEvent event) {
    ...
    public void clientInit() {
        ...
        ItemProperties.register(ACItemRegistry.SACK_OF_SATING.get(), ResourceLocation.withDefaultNamespace("open"), (stack, level, living, j) -> {
            return level != null && SackOfSatingItem.isChewing(stack, level.getGameTime()) ? 1.0F : !stack.getComponents().isEmpty() || living instanceof Player player && player.containerMenu != null && SackOfSatingItem.calculateWholeStackHungerValue(player.containerMenu.getCarried(), player) > 0 ? 0.5F : 0.0F;
        });
    ...
    private void bakeModels(final ModelEvent.ModifyBakingResult e) {
        if (AlexsCaves.CLIENT_CONFIG.emissiveBlockModels.get()) {
            ...
            for (net.minecraft.client.resources.model.ModelResourceLocation id : e.getModels().keySet()) {
                if (FULLBRIGHTS.stream().anyMatch(str -> id.id().toString().startsWith(str))) {
    ...
    private void registerShaders(final RegisterShadersEvent e) {
        try {
            ...
            e.registerShader(new ShaderInstance(e.getResourceProvider(), ResourceLocation.fromNamespaceAndPath(AlexsCaves.MODID, "rendertype_irradiated"), DefaultVertexFormat.POSITION_TEX_COLOR), ACInternalShaders::setRenderTypeIrradiatedShader);
            e.registerShader(new ShaderInstance(e.getResourceProvider(), ResourceLocation.fromNamespaceAndPath(AlexsCaves.MODID, "rendertype_blue_irradiated"), DefaultVertexFormat.POSITION_TEX_COLOR), ACInternalShaders::setRenderTypeBlueIrradiatedShader);
