...
public class ACPotionEffectLayer extends RenderLayer {
    ...
    public void render(PoseStack poseStack, MultiBufferSource bufferIn, int packedLightIn, Entity entity, float limbSwing, float limbSwingAmount, float partialTicks, float ageInTicks, float netHeadYaw, float headPitch) {
        if (entity instanceof LivingEntity living) {
            ...
            if (living.hasEffect(ACEffectRegistry.DARKNESS_INCARNATE) && AlexsCaves.CLIENT_CONFIG.radiationGlowEffect.get() && living.isAlive()) {
                ...
                int argb = ((int)(alpha * 255) << 24) | 0;
                this.getParentModel().renderToBuffer(poseStack, ivertexbuilder, 0, LivingEntityRenderer.getOverlayCoords((LivingEntity) entity, 0), argb);
                poseStack.popPose();
            }
            if (living.hasEffect(ACEffectRegistry.SUGAR_RUSH) && getParentModel() instanceof HumanoidModel<?>) {
                ...
                this.getParentModel().renderToBuffer(poseStack, ivertexbuilder, packedLightIn, LivingEntityRenderer.getOverlayCoords((LivingEntity) entity, 0), 0xFFFFFFFF);
            }
        }
    }
    ...
    private static void renderBubble(LivingEntity entity, float partialTicks, PoseStack poseStack, VertexConsumer consumer, float size, int packedLight, float textureScaleXZ, float textureScaleY, float uvOffset, boolean water) {
        ...
        renderCubeFace(entity, cubeAt, pose, consumer, packedLight, cubeStart, cubeEnd, cubeStart, cubeEnd, cubeEnd, cubeEnd, cubeEnd, cubeEnd, textureScaleXZ, textureScaleY, uvOffset, water);
        renderCubeFace(entity, cubeAt, pose, consumer, packedLight, cubeStart, cubeEnd, cubeEnd, cubeStart, cubeStart, cubeStart, cubeStart, cubeStart, textureScaleXZ, textureScaleY, uvOffset, water);
        renderCubeFace(entity, cubeAt, pose, consumer, packedLight, cubeEnd, cubeEnd, cubeEnd, cubeStart, cubeStart, cubeEnd, cubeEnd, cubeStart, textureScaleXZ, textureScaleY, uvOffset, water);
        renderCubeFace(entity, cubeAt, pose, consumer, packedLight, cubeStart, cubeStart, cubeStart, cubeEnd, cubeStart, cubeEnd, cubeEnd, cubeStart, textureScaleXZ, textureScaleY, uvOffset, water);
        renderCubeFace(entity, cubeAt, pose, consumer, packedLight, cubeStart, cubeEnd, cubeStart, cubeStart, cubeStart, cubeStart, cubeEnd, cubeEnd, textureScaleXZ, textureScaleY, uvOffset, water);
        renderCubeFace(entity, cubeAt, pose, consumer, packedLight, cubeStart, cubeEnd, cubeEnd, cubeEnd, cubeEnd, cubeEnd, cubeStart, cubeStart, textureScaleXZ, textureScaleY, uvOffset, water);
    }

    private static void renderCubeFace(LivingEntity entity, Matrix4f matrix4f, PoseStack.Pose pose, VertexConsumer vertexConsumer, int packedLightIn, float f1, float f2, float f3, float f4, float f5, float f6, float f7, float f8, float textureScaleXZ, float textureScaleY, float uvOffset, boolean water) {
        ...
        vertexConsumer.addVertex(matrix4f, f1, f3, f5).setColor(colorR, colorG, colorB, colorA).setUv((float) 0, (float) textureScaleY + uvOffset).setOverlay(overlayCoords).setLight(packedLightIn).setNormal(pose, 0.0F, -1.0F, 0.0F);
        vertexConsumer.addVertex(matrix4f, f2, f3, f6).setColor(colorR, colorG, colorB, colorA).setUv((float) textureScaleXZ, (float) textureScaleY + uvOffset).setOverlay(overlayCoords).setLight(packedLightIn).setNormal(pose, 0.0F, -1.0F, 0.0F);
