...
public class AlexsCavesPlugin implements IModPlugin {
    ...
    public static final RecipeType<RecipeHolder<CraftingRecipe>> CRAFTING_RECIPE_TYPE = RecipeTypes.CRAFTING;
    ...
    public void registerRecipes(IRecipeRegistration registration) {
        ...
        // Cast to the correct type for JEI
        List<RecipeHolder<CraftingRecipe>> caveMapRecipes = new ArrayList<>();
        for (RecipeHolder<ShapedRecipe> holder : ACRecipeMaker.createCaveMapRecipes()) {
            caveMapRecipes.add(new RecipeHolder<>(holder.id(), holder.value()));
        }
        registration.addRecipes(RecipeTypes.CRAFTING, caveMapRecipes);
        if(Minecraft.getInstance().level != null){
            List<AbstractCookingRecipe> abstractCookingRecipeList = new ArrayList<>();
            Minecraft.getInstance().level.getRecipeManager().getAllRecipesFor(NuclearFurnaceBlockEntity.getRecipeType())
                .forEach(holder -> abstractCookingRecipeList.add(holder.value()));
    ...
    public void registerRuntime(IRuntimeRegistration registration) {
        if(Minecraft.getInstance().level != null){
            var alexMealRecipe = Minecraft.getInstance().level.getRecipeManager().byKey(ResourceLocation.fromNamespaceAndPath(AlexsCaves.MODID, "alex_meal"));
            if(alexMealRecipe.isPresent() && alexMealRecipe.get().value() instanceof CraftingRecipe){
                RecipeHolder<CraftingRecipe> holder = (RecipeHolder<CraftingRecipe>) (RecipeHolder<?>) alexMealRecipe.get();
                registration.getRecipeManager().hideRecipes(RecipeTypes.CRAFTING, List.of(holder));
            }
        }
    }
