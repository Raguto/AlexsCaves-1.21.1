...
public class ACRecipeMaker {
    ...
    public static List<RecipeHolder<ShapedRecipe>> createCaveMapRecipes() {
        String group = "jei.cave_map";
        List<RecipeHolder<ShapedRecipe>> recipes = new ArrayList<>();
        for (ResourceKey<Biome> biome : ACBiomeRegistry.ALEXS_CAVES_BIOMES) {
            ...
            var pattern = ShapedRecipePattern.of(java.util.Map.of(
                    'P', paper,
                    'S', Ingredient.of(scroll)
            ), "PPP", "PSP", "PPP");
            ShapedRecipe recipe = new ShapedRecipe(group, CraftingBookCategory.MISC, pattern, map);
