...
public class LicowitchTowerStructurePiece extends UndergroundTemplateStructurePiece {
    ...
    protected void handleDataMarker(String string, BlockPos pos, ServerLevelAccessor accessor, RandomSource random, BoundingBox box) {
        if (string.equals("loot_chest")) {
            ...
            if (accessor.getBlockEntity(chestPos) instanceof RandomizableContainerBlockEntity container) {
                container.setLootTable(net.minecraft.resources.ResourceKey.create(net.minecraft.core.registries.Registries.LOOT_TABLE, ACLootTableRegistry.LICOWITCH_TOWER_CHEST), random.nextLong());
            }
        } else if (string.equals("secret_loot_chest")) {
            BlockPos chestPos = pos.below();
            if (accessor.getBlockEntity(chestPos) instanceof RandomizableContainerBlockEntity container) {
                container.setLootTable(net.minecraft.resources.ResourceKey.create(net.minecraft.core.registries.Registries.LOOT_TABLE, ACLootTableRegistry.SECRET_LICOWITCH_TOWER_CHEST), random.nextLong());
            }
        }
