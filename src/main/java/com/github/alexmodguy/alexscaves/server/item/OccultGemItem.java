...
public class OccultGemItem extends Item {
    ...
    private static void loadChunksAround(ServerLevel serverLevel, UUID ticket, BlockPos center, boolean load) {
        ...
        for (int i = -1; i <= 1; i++) {
            for (int j = -1; j <= 1; j++) {
                AlexsCaves.TICKET_CONTROLLER.forceChunk(serverLevel, ticket, chunkPos.x + i, chunkPos.z + j, load, true);
            }
        }
    }
        ...
        if (!level.isClientSide) {
            if (isActive(itemStack)) {
                ...
                if (optional.isPresent() && optional.get() == level.dimension() && compoundtag.contains("BeholderPos")) {
                    BlockPos blockpos = NbtUtils.readBlockPos(compoundtag, "BeholderPos").orElse(BlockPos.ZERO);
                    boolean flag = false;
    ...
    public InteractionResult useOn(UseOnContext context) {
        ...
        if (!level.getBlockState(blockpos).is(ACBlockRegistry.BEHOLDER.get())) {
            ...
            if (flag) {
                CompoundTag tag = itemstack.getOrDefault(net.minecraft.core.component.DataComponents.CUSTOM_DATA, net.minecraft.world.item.component.CustomData.EMPTY).copyTag();
                this.addBeholderTags(level.dimension(), blockpos, tag);
                ...
                itemstack1.set(net.minecraft.core.component.DataComponents.CUSTOM_DATA, net.minecraft.world.item.component.CustomData.of(compoundtag));
    ...
    public boolean isFoil(ItemStack stack) {
        ...
        if (!tag.isEmpty() && tag.contains("BeholderPos")) {
            Optional<ResourceKey<Level>> optional = getBeholderDimension(tag);
            Optional<BlockPos> blockpos = NbtUtils.readBlockPos(tag, "BeholderPos");
            return optional.isPresent() && blockpos.isPresent();
        }
