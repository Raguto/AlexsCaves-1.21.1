...
public class ForsakenRandomlyJumpGoal extends Goal {
    ...
    private BlockPos findJumpTarget() {
        ...
        if (vec3 != null) {
            ...
            if (entity.level().getBlockState(blockpos.below()).isSolidRender(entity.level(), blockpos.below()) && entity.getPathfindingMalus(WalkNodeEvaluator.getPathTypeStatic(entity, blockpos)) == 0.0F && entity.level().isUnobstructed(this.entity, Shapes.create(aabb))) {
                return blockpos;
            }
        }
        return null;
