...
public class MagnetUtil {
    ...
    public static void tickMagnetism(Entity entity) {
        ...
        if (vec3 != Vec3.ZERO) {
            ...
            if (entity instanceof LivingEntity living) {
                if (living.onGround() && standingOnDirection == dir && living.getDeltaMovement().y > 0.1) {
                    // Player is trying to jump
                    if (living.level().isClientSide) {
                        AlexsCaves.sendMSGToServer(new PlayerJumpFromMagnetMessage(living.getId(), true));
                    }
    ...
    private static Vec3 processMovementControls(float dist, LivingEntity living, Direction dir) {
        ...
        // Check if entity is trying to jump by looking at vertical movement intent
        float jump = getStandingOnMagnetSurface(living) != null && living.getDeltaMovement().y > 0.1 ? 0.75F : -0.1F;
