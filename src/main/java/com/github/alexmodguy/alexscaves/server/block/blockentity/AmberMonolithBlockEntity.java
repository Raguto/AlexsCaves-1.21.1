    ...
    private boolean spawnMobs() {
        ...
        for (int l1 = 0; l1 < spawnCount; l1++) {
            ...
            for (int i2 = 0; !flag && i2 < 6; i2++) {
                ...
                if (spawnType.canSummon() && SpawnPlacements.checkSpawnRules(spawnType, (ServerLevelAccessor) level, MobSpawnType.SPAWNER, blockpos, level.getRandom())) {
                    double d0 = blockpos.getX() + 0.5F;
                    double d1 = blockpos.getZ() + 0.5F;
                    if (!level.noCollision(spawnType.getSpawnAABB(d0, (double) blockpos.getY(), d1))) {
                        continue;
                    }
                    ...
                    if (entity instanceof Mob) {
                        Mob mob = (Mob) entity;
                        // ForgeEventFactory.checkSpawnPosition removed in 1.21 - just check spawn rules directly
                        if (SpawnPlacements.checkSpawnRules(spawnType, (ServerLevelAccessor) level, MobSpawnType.CHUNK_GENERATION, blockpos, level.getRandom())) {
