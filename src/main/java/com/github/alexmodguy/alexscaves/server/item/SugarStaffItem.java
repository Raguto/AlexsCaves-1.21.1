    ...
    public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
        ...
        if(!level.isClientSide){
            ...
            if(hex){
                ...
                sugarStaffHexEntity.setHexScale(1.0F + 0.25F * itemstack.getEnchantmentLevel(player.registryAccess().lookupOrThrow(net.minecraft.core.registries.Registries.ENCHANTMENT).getOrThrow(ACEnchantmentRegistry.HUMUNGOUS_HEX)));
                ...
                sugarStaffHexEntity.setLifespan(100 + 60 * itemstack.getEnchantmentLevel(player.registryAccess().lookupOrThrow(net.minecraft.core.registries.Registries.ENCHANTMENT).getOrThrow(ACEnchantmentRegistry.SPELL_LASTING)));
                player.getCooldowns().addCooldown(this, 100);
            }else{
                int spawnIn = 3 + itemstack.getEnchantmentLevel(player.registryAccess().lookupOrThrow(net.minecraft.core.registries.Registries.ENCHANTMENT).getOrThrow(ACEnchantmentRegistry.MULTIPLE_MINT));
                boolean flag = false;
                int despawnTime = 80;
                for (int i = 0; i < spawnIn; i++) {
                    ...
                    if(itemstack.getEnchantmentLevel(player.registryAccess().lookupOrThrow(net.minecraft.core.registries.Registries.ENCHANTMENT).getOrThrow(ACEnchantmentRegistry.PEPPERMINT_PUNTING)) > 0){
                    ...
                    if(lookingAtEntity != null && itemstack.getEnchantmentLevel(player.registryAccess().lookupOrThrow(net.minecraft.core.registries.Registries.ENCHANTMENT).getOrThrow(ACEnchantmentRegistry.SEEKCANDY)) > 0){
